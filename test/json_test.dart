...
void testJson(jsonText, expected) {
  expect(jsonc.decode(jsonText), equals(expected));
  expect(jsoncDecode(jsonText), equals(expected));
}
...
void testThrows(jsonText) {
  expect(() => jsonc.decode(jsonText), throwsFormatException);
  expect(() => jsoncDecode(jsonText), throwsFormatException);
  expect(() => jsonc.decoder.convert(jsonText), throwsFormatException);
  expect(
      () => convert.utf8.decoder
          .fuse(jsonc.decoder)
          .convert(convert.utf8.encode(jsonText)),
